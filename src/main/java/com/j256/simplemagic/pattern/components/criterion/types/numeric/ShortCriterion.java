...
/**
 * Represents a two-byte numeric criterion from a line in magic (5) format.
 *
 * @author graywatson
 */
public class ShortCriterion extends AbstractNumberCriterion {

	/**
	 * Creates a new {@link ShortCriterion} as found in a {@link MagicPattern}. The criterion shall define one Short
	 ...
	 */
	public ShortCriterion(EndianType endianness) throws MagicPatternException {
		super(endianness);
	}
	...
	protected Number applyConjunction(Number extractedValue, Number operand) {
		return extractedValue.shortValue() & operand.shortValue();
	}
	...
	protected Number applyComplement(Number extractedValue) {
		return ~extractedValue.shortValue();
	}
	...
	protected boolean testEqual(Number extractedValue) throws MagicPatternException {
		...
		return getTestValue() != null && extractedValue != null &&
				extractedValue.shortValue() == getTestValue().shortValue();
	}
	...
	protected boolean testComplement(Number extractedValue) {
		return (extractedValue.longValue() == (~getTestValue().longValue() & 0xFFFFL));
	}
}
