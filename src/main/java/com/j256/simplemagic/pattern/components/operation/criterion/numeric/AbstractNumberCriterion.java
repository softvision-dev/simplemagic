...
public abstract class AbstractNumberCriterion extends AbstractMagicCriterion<Number> {
	...
	public static final MagicOperator[] TYPE_MODIFIERS = new MagicOperator[]{
			MagicOperator.CONJUNCTION, MagicOperator.DISJUNCTION, MagicOperator.CONTRAVALENCE, MagicOperator.COMPLEMENT,
			MagicOperator.ADD, MagicOperator.SUBTRACT, MagicOperator.MULTIPLY, MagicOperator.DIVIDE, MagicOperator.MODULO
	};
	...
	//TODO: Implementation of "apply" methods is naive for most numeric types, test and check for validity of results.
	...
	public MagicCriterionResult<Number> isMatch(byte[] data, int currentReadOffset, boolean invertEndianness)
		...
		if (extractor instanceof NumberExtractor) {
			extractedValue = ((NumberExtractor) extractor).extractValue(data, currentReadOffset, invertEndianness);
		}
