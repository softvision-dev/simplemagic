...
/**
 * Represents a Byte criterion from a line in magic (5) format.
 * <p>
 * A one-byte value.
 * </p>
 *
 * @author graywatson
 */
public class ByteCriterion extends AbstractNumberCriterion {
	...
	protected Number applyConjunction(Number extractedValue, Number operand) {
		return extractedValue.byteValue() & operand.byteValue();
	}
	...
	protected Number applyComplement(Number extractedValue) {
		return ~extractedValue.byteValue();
	}
	...
	protected boolean testEqual(Number extractedValue) throws MagicPatternException {
		...
		return getTestValue() != null && extractedValue != null &&
				extractedValue.byteValue() == getTestValue().byteValue();
	}
	...
	protected boolean testComplement(Number extractedValue) {
		return getTestValue() != null && extractedValue != null &&
				(extractedValue.longValue() == ((~getTestValue().longValue()) & 0xFFL));
	}
}
