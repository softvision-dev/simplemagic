...
public class String16Criterion extends StringCriterion {
	 ...
	 */
	@Override
	public MagicCriterionResult<String, StringOperator> isMatch(byte[] data, int currentReadOffset) throws MagicPatternException {
		...
		MagicCriterionResult<String, StringOperator> result = super.findOffsetMatch(
				null, extractedValue, 0, extractedValue.length
		);
		if (result.isMatch()) {
			return new MagicCriterionResult<String, StringOperator>(
